...
docker cp minikube:opt/cni/bin /custom-node/opt/cni/
docker cp minikube:/etc/cni/net.d /custom-node/etc/cni/
...
echo "Labeling node.."
kubectl --kubeconfig=/home/ish/.kube/config label no custom-node kubernetes.io/role=worker